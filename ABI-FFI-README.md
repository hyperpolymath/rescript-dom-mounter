...
│  generated/abi/rescript-dom-mounter.h                │
└─────────────────┬───────────────────────────┘
                  │
                  │ imported by
                  ▼
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to librescript-dom-mounter.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── rescript-dom-mounter.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── rescript-dom-mounter.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/rescript-dom-mounter.h
```
...
```c
#include "rescript-dom-mounter.h"

int main() {
    void* handle = rescript-dom-mounter_init();
    if (!handle) return 1;

    int result = rescript-dom-mounter_process(handle, 42);
    if (result != 0) {
        const char* err = rescript-dom-mounter_last_error();
        fprintf(stderr, "Error: %s\n", err);
    }

    rescript-dom-mounter_free(handle);
    return 0;
}
```
...
Compile with:
```bash
gcc -o example example.c -lrescript-dom-mounter -L./zig-out/lib
```
...
```rust
#[link(name = "rescript-dom-mounter")]
extern "C" {
    fn rescript-dom-mounter_init() -> *mut std::ffi::c_void;
    fn rescript-dom-mounter_free(handle: *mut std::ffi::c_void);
    fn rescript-dom-mounter_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = rescript-dom-mounter_init();
        assert!(!handle.is_null());

        let result = rescript-dom-mounter_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const librescript-dom-mounter = "librescript-dom-mounter"

function init()
    handle = ccall((:rescript-dom-mounter_init, librescript-dom-mounter), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:rescript-dom-mounter_process, librescript-dom-mounter), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:rescript-dom-mounter_free, librescript-dom-mounter), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/rescript-dom-mounter.h
   ```
