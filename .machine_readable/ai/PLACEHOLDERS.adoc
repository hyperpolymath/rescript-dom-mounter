...
sed -i 's/rescript-dom-mounter/my-project/g' $(grep -rl 'rescript-dom-mounter' .)
sed -i 's/rescript-dom-mounter/MY_PROJECT/g' $(grep -rl 'rescript-dom-mounter' .)
sed -i 's/rescript-dom-mounter/my_project/g' $(grep -rl 'rescript-dom-mounter' .)
sed -i 's/rescript-dom-mounter/my-project/g' $(grep -rl 'rescript-dom-mounter' .)
...
| Placeholder | Description | Example | Files |
|---|---|---|---|
| `rescript-dom-mounter` | Human-readable project name | `My Project` | SECURITY.md, CODE_OF_CONDUCT.md, TOPOLOGY.md, STATE.a2ml, Justfile, GOVERNANCE.md, MAINTAINERS.md, flake.nix, devcontainer.json |
| `{{PROJECT_DESCRIPTION}}` | One-line description | `A tool for X` | flake.nix |
| `rescript-dom-mounter` | Uppercase identifier (for Idris2 modules, C macros) | `MY_PROJECT` | ABI-FFI-README.md, src/interface/abi/*.idr, src/interface/ffi/*.zig |
| `rescript-dom-mounter` | Lowercase identifier (for C symbols, filenames) | `my_project` | ABI-FFI-README.md, src/interface/ffi/*.zig |
| `rescript-dom-mounter` | Repository name (slug) | `my-project` | CONTRIBUTING.md, SECURITY.md, CODE_OF_CONDUCT.md, cliff.toml |
