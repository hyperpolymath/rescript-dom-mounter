...
= rescript-dom-mounter Container Templates
:toc: left
:toclevels: 3
:sectnums:
...
All files use `rescript-dom-mounter` tokens that are replaced by `just container-init`
...
(service name, port, registry) and substitutes all `rescript-dom-mounter`
